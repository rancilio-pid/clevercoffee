...
// taken from sample code from datasheet, adapted parameters for model ABP2LANT010BG2A3XX
...
float ABP2_measurePressure()
{
    ...
    Wire.requestFrom(ABP2_id, (uint8_t)7); // read back Sensor data 7 bytes
    int i = 0;
    for (i = 0; i < 7; i++)
    {
        ABP2_data[i] = Wire.read();
    }
    ABP2_press_counts = (double)(ABP2_data[3] + ABP2_data[2] * 256 + ABP2_data[1] * 65536); // calculate digital pressure counts
    ABP2_temp_counts = (double)(ABP2_data[6] + ABP2_data[5] * 256 + ABP2_data[4] * 65536);  // calculate digital temperature counts
    ABP2_temperature = (ABP2_temp_counts * 270.0 / 16777215.0) - 40.0;        // calculate temperature in deg c
    ABP2_percentage = (ABP2_press_counts / 16777215.0) * 100.0;             // calculate pressure as percentage of full scale
    // calculation of pressure value according to equation 2 of datasheet
    ABP2_pressure = ((ABP2_press_counts - ABP2_outputmin) * (ABP2_pmax - ABP2_pmin)) / (ABP2_outputmax - ABP2_outputmin) + ABP2_pmin;
#ifdef PRESSURE_VERBOSE
