...
void printScreen() {
    ...
    if (((machineState == kPidNormal || machineState == kBrewDetectionTrailing) || ((machineState == kBrew || machineState == kShotTimerAfterBrew) && FEATURE_SHOTTIMER == 0) || ((machineState == kPidDisabled) && FEATURE_PIDOFF_LOGO == 0)) &&
        (brewSwitchState != kBrewSwitchFlushOff) || (machineState == kWaterEmpty) || (machineState == kPidDisabled) || (machineState == kStandby)|| (machineState == kSteam)) {
