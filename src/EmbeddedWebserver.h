...
void serverSetup()
{
    ...
    // serve static files
    int maxAge = 60 * 60 * 24 * 7; // max Age for Cache

    LittleFS.begin();
    server.serveStatic("/manifest.json", LittleFS, "/pwa/manifest.json", "max-age=" + maxAge);
    server.serveStatic("/css", LittleFS, "/css/", "max-age=604800" + maxAge);
    server.serveStatic("/js", LittleFS, "/js/", "max-age=604800" + maxAge);
    server.serveStatic("/img", LittleFS, "/img/", "max-age=604800" + maxAge);
    server.serveStatic("/webfonts", LittleFS, "/webfonts/", "max-age=604800" + maxAge);
    server.serveStatic("/", LittleFS, "/html/", "max-age=604800" + maxAge).setDefaultFile("index.html").setTemplateProcessor(staticProcessor);
