...
std::vector<editable_t> editableVars = {
    ...
    {"START_KP", "Start Kp", "Proportional gain for cold start controller (PonM mode, <a href='http://brettbeauregard.com/blog/2017/06/introducing-proportional-on-measurement/' target='_blank'>details</a>). This value is not used with the the error as usual but the absolute value of the temperature and counteracts the integral part as the temperature rises. Ideally, both parameters are set so that they balance each other out when the target temperature is reached.", kDouble, 0, true, (void *)&startKp},
    {"START_TN", "Start Tn", "Integral gain for cold start controller (PonM mode, <a href='http://brettbeauregard.com/blog/2017/06/introducing-proportional-on-measurement/' target='_blank'>details</a>)", kDouble, 0, true, (void *)&startTn},
    {"PID_KP", "PID Kp", "Proportional gain (in Watts/C°) for the main PID controller (in P-Tn-Tv form, <a href='http://testcon.info/EN_BspPID-Regler.html#strukturen' target='_blank'>Details<a>). The higher this value is, the higher is the output of the heater for a given temperature difference. E.g. 5°C difference will result in P*5 Watts of heater output.", kDouble, 0, true,(void *)&aggKp},
    {"PID_TN", "PID Tn (=Kp/Ki)", "Integral time constant (in seconds) for the main PID controller (in P-Tn-Tv form, <a href='http://testcon.info/EN_BspPID-Regler.html#strukturen' target='_blank'>Details<a>). The larger this value is, the slower the integral part of the PID will increase (or decrease) if the process value remains above (or below) the setpoint in spite of proportional action. The smaller this value, the faster the integral term changes.", kDouble, 0, true, (void *)&aggTn},
    {"PID_TV", "PID Tv (=Kd/Kp)", "Differential time constant (in seconds) for the main PID controller (in P-Tn-Tv form, <a href='http://testcon.info/EN_BspPID-Regler.html#strukturen' target='_blank'>Details<a>). This value determines how far the PID equation projects the current trend into the future. The higher the value, the greater the dampening. Select it carefully, it can cause oscillations if it is set too high or too low.", kDouble, 0, true, (void *)&aggTv},
    {"PID_I_MAX", "PID Integrator Max", "Internal integrator limit to prevent windup (in Watts). This will allow the integrator to only grow to the specified value. This should be approximally equal to the output needed to hold the temperature after the setpoint has been reached and is depending on machine type and whether the boiler is insulated or not.", kDouble, 0, true, (void *)&aggIMax},
    {"STEAM_KP", "Steam Kp", "Proportional gain for the steaming mode (I or D are not used)", kDouble, 0, true, (void *)&steamKp},
    {"TEMP", "Temperature", "", kDouble, 0, false, (void *)&Input},
    {"BREW_SET_POINT", "Set point (°C)", "The temperature that the PID will attempt to reach and hold", kDouble, 1, true, (void *)&BrewSetPoint},
    {"BREW_TEMP_OFFSET", "Offset (°C)", "Optional offset that is added to the user-visible setpoint. Can be used to compensate sensor offsets and the average temperature loss between boiler and group so that the setpoint represents the approximate brew temperature.", kDouble, 1, true, (void *)&BrewTempOffset},
    {"BREW_TIME", "Brew Time (s)", "", kDouble, 1, true, (void *)&brewtime},
    {"BREW_PREINFUSIONPAUSE", "Preinfusion Pause Time (s)", "", kDouble, 1, true, (void *)&preinfusionpause},
    {"BREW_PREINFUSION", "Preinfusion Time (s)", "", kDouble, 1, true, (void *)&preinfusion},
    {"SCALE_WEIGHTSETPOINT", "Brew weight setpoint (g)", "Only used when a scale is built in", kDouble, 1, true, (void *)&weightSetpoint},
    {"PID_BD_KP", "BD Kp", "Proportional gain (in Watts/°C) for the PID when brewing has been detected. Use this controller to either increase heating during the brew to counter temperature drop from fresh cold water in the boiler. Some machines, e.g. Rancilio Silvia, actually need to heat less not at all during the brew because of high temperature stability (<a href='https://www.kaffee-netz.de/threads/installation-eines-temperatursensors-in-silvia-bruehgruppe.111093/#post-1453641' target='_blank'>Details<a>)", kDouble, 2, true, (void *)&aggbKp},
    {"PID_BD_TN", "BD Tn (=Kp/Ki)", "Integral time constant (in seconds) for the PID when brewing has been detected.", kDouble, 2, true, (void *)&aggbTn},
    {"PID_BD_TV", "BD Tv (=Kd/Kp)", "Differential time constant (in seconds) for the PID when brewing has been detected.", kDouble, 2, true, (void *)&aggbTv},
    {"PID_BD_TIMER", "PID BD Time (s)", "Fixed time in seconds for which the BD PID will stay enabled (also after Brew switch is inactive again).", kDouble, 2, true, (void *)&brewtimersoftware},
    {"PID_BD_BREWSENSITIVITY", "PID BD Sensitivity", "Software brew detection sensitivity that looks at average temperature, <a href='https://manual.rancilio-pid.de/de/customization/brueherkennung.html' target='_blank'>Details</a>. Needs to be &gt;0 also for Hardware switch detection.", kDouble, 2, true, (void *)&brewsensitivity},
    {"STEAM_MODE", "Steam Mode", "", rInteger, 0, false, (void *)&SteamON},
    {"BACKFLUSH_ON", "Backflush", "", rInteger, 0, false, (void *)&backflushON},
};
