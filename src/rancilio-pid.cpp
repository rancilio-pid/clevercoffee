...
std::vector<editable_t> editableVars = {
    ...
    {"PID_I_MAX", "PID Integrator Max", "Internal integrator limit to prevent windup (in Watts). This will allow the integrator to only grow to the specified value. This should be approximally equal to the output needed to hold the temperature after the setpoint has been reached and is depending on machine type and whether the boiler is insulated or not.", kDouble, (void *)&aggIMax},
    ...
    {"STEAM_KP", "Steam Kp", "Proportional gain for the steaming mode (I or D are not used)", kDouble, (void *)&steamKp}
};
