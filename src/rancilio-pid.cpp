...
std::vector<editable_t> editableVars = {
    ...
    {"START_KP", "Start Kp", "Proportional gain for cold start controller (PonM mode, <a href='http://brettbeauregard.com/blog/2017/06/introducing-proportional-on-measurement/' target='_blank'>details</a>). This value is not used with the the error as usual but the absolute value of the temperature and counteracts the integral part as the temperature rises. Ideally, both parameters are set so that they balance each other out when the target temperature is reached.", kDouble, sPIDSection, true, (void *)&startKp},
    {"START_TN", "Start Tn", "Integral gain for cold start controller (PonM mode, <a href='http://brettbeauregard.com/blog/2017/06/introducing-proportional-on-measurement/' target='_blank'>details</a>)", kDouble, sPIDSection, true, (void *)&startTn},
    ...
    {"BREW_TIME", "Brew Time (s)", "", kDouble, sTempSection, true, (void *)&brewtime},
    {"BREW_PREINFUSIONPAUSE", "Preinfusion Pause Time (s)", "", kDouble, sTempSection, true, (void *)&preinfusionpause},
    {"BREW_PREINFUSION", "Preinfusion Time (s)", "", kDouble, sTempSection, true, (void *)&preinfusion},
    {"SCALE_WEIGHTSETPOINT", "Brew weight setpoint (g)", "Only used when a scale is built in", kDouble, sTempSection, true, (void *)&weightSetpoint},
    {"PID_BD_KP", "BD Kp", "Proportional gain (in Watts/°C) for the PID when brewing has been detected. Use this controller to either increase heating during the brew to counter temperature drop from fresh cold water in the boiler. Some machines, e.g. Rancilio Silvia, actually need to heat less not at all during the brew because of high temperature stability (<a href='https://www.kaffee-netz.de/threads/installation-eines-temperatursensors-in-silvia-bruehgruppe.111093/#post-1453641' target='_blank'>Details<a>)", kDouble, sBDSection, true, (void *)&aggbKp},
    {"PID_BD_TN", "BD Tn (=Kp/Ki)", "Integral time constant (in seconds) for the PID when brewing has been detected.", kDouble, sBDSection, true, (void *)&aggbTn},
    {"PID_BD_TV", "BD Tv (=Kd/Kp)", "Differential time constant (in seconds) for the PID when brewing has been detected.", kDouble, sBDSection, true, (void *)&aggbTv},
    {"PID_BD_TIMER", "PID BD Time (s)", "Fixed time in seconds for which the BD PID will stay enabled (also after Brew switch is inactive again).", kDouble, sBDSection, true, (void *)&brewtimersoftware},
    {"PID_BD_BREWSENSITIVITY", "PID BD Sensitivity", "Software brew detection sensitivity that looks at average temperature, <a href='https://manual.rancilio-pid.de/de/customization/brueherkennung.html' target='_blank'>Details</a>. Needs to be &gt;0 also for Hardware switch detection.", kDouble, sBDSection, true, (void *)&brewsensitivity},
