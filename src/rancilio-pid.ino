...
/**
 * Switch to offline modeif maxWifiReconnects were exceeded during boot
 */
 ...
 */
void checkBlynk() {
  if (Offlinemodus == 1 || BLYNK == 0 || brewcounter > 11)
    return;
...
/**
 * Convert double, float int and uint to char for MQTT Publish
 */
...
void machinestatevoid() {
  ...
  case kColdStart:
    ...
    case kBrew:
      ...
      if ((bezugsZeit > 35 * 1000 && Brewdetection == 1 && ONLYPID == 1) || // 35 sec later and BD PID active SW Solution
    ...
    case kBrewDetectionTrailing:
      ...
      if ((bezugsZeit > 0 && ONLYPID == 1 && Brewdetection == 3) || // New Brew inner BD only by Only PID AND Voltage Sensor
          (ONLYPID == 0 && brewcounter > 10 && brewcounter <= 42))
      {
        machinestate = kBrew;
      }
