...
int lastStateSteamTrigger;                          // the current reading from the input pin
...
void checkSteamSwitch() {
        ...
        if ((millis() - lastDebounceTimeSteamTrigger) > debounceDelaySteamTrigger) {
            ...
            if (reading != lastStateSteamTrigger) {
                lastStateSteamTrigger = reading;

                // only toggle heating power if the new button state is HIGH
                if (lastStateSteamTrigger == HIGH) {
                    if (steamON == 0) {
