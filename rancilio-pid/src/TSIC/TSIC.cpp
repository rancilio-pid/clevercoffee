	...
	: m_signal_pin(signal_pin), m_vcc_pin(vcc_pin), m_sens_type(sens_type)
{
    ...
    wait_time = timeout_tmp = 0;
    start = end = readSens1 = readSens2 = 0 ;
}
...
uint8_t TSIC::readSens(uint16_t *temp_value){
	...
	unsigned long start_internal;
	unsigned long end_internal;
	
	start_internal = micros();
	while (TSIC_HIGH){	// wait until start bit starts  //74000 micros
		timeout++;
		delayMicroseconds(10);
		Cancel();
	}
	if (flag) {
		end_internal = micros();
		timeout_tmp = end_internal - start_internal;
		wait_time = end_internal - start_internal;
		data_start_time = end_internal;
		flag = false;
	}
	...
	start_internal = micros();
	while (TSIC_LOW) {    // wait for rising edge // 40micros -> 55micros
		//if (flag) timeout_tmp++; 
		strobelength++;
		timeout++;
		delayMicroseconds(10);
	...
	if (flag && false) {
        ...
        //Serial.printf("B: Waited %d micro sec\n", (timeout -9900) * 10);
	for (uint8_t i=0; i<9; i++) {
		// Wait for bit start
		timeout = 0;
		start_internal = micros();
		while (TSIC_HIGH) { // wait for falling edge  //45micros
			timeout++;
			Cancel();
		}
		if (flag && false) {
		...
		*temp_value <<= 1;
		...
		timeout = 0;
		start_internal = micros();
		while (TSIC_LOW) {		// wait for rising edge  //26 micros
			timeout++;
			Cancel();
		}
		if (flag && false) {
